...
const MaterialsGrid = () => {
  ...
  const fetchMaterials = async () => {
    try {
      ...
      // Obter o token de acesso usando a nova API do Supabase
      ...
      // Buscar materiais usando a nova rota da API
      const response = await fetch('https://edumanager-backend-5olt.onrender.com/api/aluno/materiais', {
        headers: {
      ...
      // Atualizar dados com a resposta da API
      if (data.data.professor) {
        setProfessorInfo({
          id: data.data.professor.id,
          nome: data.data.professor.nome
        });
        setMaterials(data.data.materiais || []);
      } else {
