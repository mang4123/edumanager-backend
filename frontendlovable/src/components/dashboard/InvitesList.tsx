...
const InvitesList = () => {
  ...
  const copyInviteLink = async (token: string, nomeAluno: string) => {
    try {
      ...
      toast({
        title: "Link copiado!",
        description: `Link do convite para ${nomeAluno} copiado para a área de transferência`,
      });
    } catch (error) {
      toast({
        title: "Erro",
  ...
  const deleteInvite = async (id: string, nomeAluno: string) => {
    try {
      ...
      toast({
        ...
        description: `Convite para ${nomeAluno} foi removido com sucesso`,
      });
    } catch (error) {
              ...
              >
                ...
                <div className="flex gap-2 pt-2 border-t">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => window.open(`${window.location.origin}/register?invite=${convite.token}`, '_blank')}
                    disabled={convite.status !== 'pendente' || new Date(convite.expires_at) < new Date()}
                  >
