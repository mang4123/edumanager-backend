...
const TeacherDashboard = () => {
  ...
  // CORREÇÃO: Removido 'students' do estado (StudentsGrid agora busca dados internamente)
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // CORREÇÃO: Melhorado cálculo de receita mensal (apenas do mês atual)
      const agora = new Date();
      const mesAtualNum = agora.getMonth();
      const anoAtualNum = agora.getFullYear();
      
      const receitaMensal = financeiro?.filter(f => {
        const dataItem = new Date(f.created_at);
        return f.status === 'pago' && 
               dataItem.getMonth() === mesAtualNum && 
               dataItem.getFullYear() === anoAtualNum;
      ...
      // CORREÇÃO: Removido código de montagem de lista de estudantes 
      // (StudentsGrid agora busca dados internamente)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="students" className="space-y-6">
            {/* CORREÇÃO: Removida prop 'students' - StudentsGrid agora busca dados internamente */}
            <StudentsGrid />
            <InvitesList />
          </TabsContent>
