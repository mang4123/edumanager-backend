...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const createAndEnsureUserProfile = async (authUser: User) => {
    ...
    try {
      ...
      try {
        console.log('🔍 [DEBUG] Iniciando verificação na tabela profiles...');
        ...
        console.log('📊 [DEBUG] Resultado da consulta profiles:', { existingProfile, fetchError });

        if (fetchError && fetchError.code === 'PGRST116') {
          // Perfil não existe, criar um
          console.log('📝 [DEBUG] Perfil não existe, criando na tabela profiles...');
          ...
          if (insertError) {
            ...
            console.log('✅ [DEBUG] Perfil criado na tabela profiles com sucesso');
          }
        } else if (!fetchError) {
          console.log('✅ [DEBUG] Perfil já existe na tabela profiles');
        } else {
          console.error('⚠️ [DEBUG] Erro inesperado ao verificar perfil:', fetchError);
        }
        
        console.log('🏁 [DEBUG] Verificação profiles concluída');
        
      } catch (profileError) {
        console.error('💥 [DEBUG] Exception ao verificar/criar perfil:', profileError);
      }
